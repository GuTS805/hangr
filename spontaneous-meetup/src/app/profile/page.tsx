...
export default function ProfilePage() {
  ...
  const [safetyTimerMins, setSafetyTimerMins] = useState(() =>
    typeof window !== "undefined" ? parseInt(localStorage.getItem("hangr_safety_timer_mins") ?? "0") : 0
  );
  const [verifiedPingsOnly, setVerifiedPingsOnly] = useState(() =>
    typeof window !== "undefined" ? localStorage.getItem("hangr_verified_pings_only") === "true" : false
  );
          ...
          <div className="flex-1 overflow-y-auto px-5 sm:px-6 py-5 space-y-5">
            ...
            {rightTab === "safety" && (
              <>
                ...
                {/* Auto-SOS Timer */}
                <div>
                  <p className="text-sm font-semibold text-gray-800 mb-1">Auto-SOS Timer</p>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {([{ label: "Off", value: 0 }, { label: "30 min", value: 30 }, { label: "1 hr", value: 60 }, { label: "2 hrs", value: 120 }, { label: "3 hrs", value: 180 }] as { label: string; value: number }[]).map(({ label, value }) => (
                      <button
                        key={value}
                        onClick={() => { setSafetyTimerMins(value); localStorage.setItem("hangr_safety_timer_mins", String(value)); }}
                        className={`px-3 py-1.5 rounded-full text-xs font-semibold transition-colors ${
                          safetyTimerMins === value ? "bg-red-500 text-white" : "bg-gray-100 text-gray-600 hover:bg-gray-200"
                        }`}
                      >
                        {label}
                      </button>
                  ...
                  {safetyTimerMins > 0 && !emergencyName && (
                    <p className="text-xs text-orange-500 mt-2">⚠️ Add an emergency contact above to activate this timer</p>
                  )}
                  {safetyTimerMins > 0 && emergencyName && (
                    <p className="text-xs text-green-600 mt-2">✓ Timer active — will auto-fire SOS to {emergencyName} if you don&apos;t check in</p>
                  )}
                </div>

                {/* Women's safety section */}
                {currentUser.gender === "Female" && (
                  <div className="bg-pink-50 border border-pink-200 rounded-2xl px-4 py-4 space-y-4">
                    <div className="flex items-center gap-2">
                      <span className="text-pink-500">♀</span>
                      <p className="text-sm font-semibold text-pink-800">Women&apos;s Safety Settings</p>
                    </div>
                    ...
                    <label className="flex items-start gap-3 cursor-pointer">
                      <input
                        type="checkbox"
                        checked={verifiedPingsOnly}
                        onChange={(e) => {
                          setVerifiedPingsOnly(e.target.checked);
                          localStorage.setItem("hangr_verified_pings_only", String(e.target.checked));
                        }}
                        className="w-4 h-4 mt-0.5 accent-pink-500 flex-shrink-0"
                      />
                      <div>
                        <p className="text-xs font-semibold text-pink-800">Only show pings from verified users</p>
                        <p className="text-xs text-pink-500 mt-0.5">Pings from unverified accounts are hidden from your inbox</p>
                      </div>
                    </label>

                    {/* Safety tips */}
                    <div className="space-y-1.5 pt-1 border-t border-pink-200">
                      <p className="text-xs font-semibold text-pink-700">Before every meetup</p>
                      {[
                        ...
                        <div key={tip} className="flex items-start gap-1.5">
                          ...
                          <p className="text-xs text-pink-600">{tip}</p>
                        </div>
                      ))}
                    </div>
                  </div>
